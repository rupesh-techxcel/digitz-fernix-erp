...
let calendar = null;               // Holds the calendar instance
let fullcalendar_loaded = false;  // Tracks whether assets are loaded
...
// 📦 Frappe Form Trigger
frappe.ui.form.on('Customer Calendar', {
    refresh(frm) {
        setupTooltip();
...
function render_calendar(frm) {
    const wrapper = frm.fields_dict.calendar_html.$wrapper;
    wrapper.html(`<div id="custom-calendar" style="width:100%; min-height:600px;"></div>`);

    load_calendar_assets(() => {
        frappe.call({
            method: "digitz_erp.api.customer_api.get_customer_schedules",
            args: { docname: frm.doc.name },
            callback: function (r) {
               if (!r.message) return;

                const schedules = r.message.receipts || [];
                ...
                for (const [date, data] of Object.entries(groupedByDate)) {
                    ...
                    data.items.forEach(schedule => {
                        scheduleEvents.push({
                           title: schedule.amount ? `${schedule.amount} ${currency} - ${schedule.customer?.slice(0, 23)}...` : '',
                            start: schedule.scheduled_date,
                            color: "#28a745",
                            textColor: "#ffffff",
                             order_priority: 1, // ⬅️ will appear after "Total"
                            extendedProps: {
                                description: schedule.customer,
...
function load_calendar_assets(callback) {
    if (fullcalendar_loaded || typeof FullCalendar !== "undefined") {
